...
class Paid_Listings {
	...
	public function display_listing_in_cart( $data, $cart_item ) {
		if ( isset( $cart_item['job_id'] ) ) {
			...
			if ( ! empty( $cart_item['assignment_type'] ) ) {
				switch ( $cart_item['assignment_type'] ) {
					case 'claim':
						$name = esc_html__( 'Claim for', 'my-listing' );
						break;
					case 'switch':
						$name = esc_html__( 'Switch package for', 'my-listing' );
						break;
					case 'relist':
						$name = esc_html__( 'Relist for', 'my-listing' );
						break;
					default:
						break;
				}
	...
	public function woocommerce_thankyou( $order_id ) {
		...
		foreach ( $order->get_items() as $item ) {
			...
			$is_switch = ! empty( $item['assignment_type'] ) && $item['assignment_type'] === 'switch';
			$is_relist = ! empty( $item['assignment_type'] ) && $item['assignment_type'] === 'relist';

			if ( $is_switch ) {
				switch( get_post_status( $item['job_id'] ) ):
					case 'pending':
						echo wpautop( sprintf( __( 'The plan for %s has been updated. Your listing will be visible once approved.', 'my-listing' ), get_the_title( $item['job_id'] ) ) );
					break;
					case 'pending_payment':
						echo wpautop( sprintf( __( 'The plan switch for %s has been submitted successfully. The listing will be updated once the payment is confirmed.', 'my-listing' ), get_the_title( $item['job_id'] ) ) );
					break;
					default :
						echo wpautop( sprintf( __( 'The plan for %s has been updated.', 'my-listing' ), get_the_title( $item['job_id'] ) ) );
					break;
				endswitch;
			} elseif ( $is_claim ) {
				if ( $is_paid ) {
				...
				switch ( get_post_status( $item['job_id'] ) ) {
					case 'pending' :
						echo wpautop( sprintf( __( '%s has been relisted successfully and will be visible once approved.', 'my-listing' ), get_the_title( $item['job_id'] ) ) );
					break;
					case 'pending_payment' :
						echo wpautop( sprintf( __( '%s has been submitted for relisting and will be visible once payment has been confirmed.', 'my-listing' ), get_the_title( $item['job_id'] ) ) );
					break;
					default :
						echo wpautop( sprintf( __( '%s has been relisted successfully.', 'my-listing' ), get_the_title( $item['job_id'] ) ) );
					break;
				}
			} else {
				switch ( get_post_status( $item['job_id'] ) ) {
